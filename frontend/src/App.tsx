...
import { LayoutDashboard, Users, UserPlus, Menu, X, Bell, ChevronDown, LogOut, Settings, Check, Building2 } from 'lucide-react';
...
const SidebarItem = ({ to, icon: Icon, label, active }: { to: string, icon: any, label: string, active: boolean }) => (
  <Link
    to={to}
    className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${active ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-600 hover:bg-gray-100'
      }`}
  >
    <Icon size={20} />
          ...
          <div className="flex items-center gap-4">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <div className="flex items-center gap-2 px-3 py-1.5 border rounded-md cursor-pointer hover:bg-gray-50 group transition-all">
                  <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Branch:</span>
                  <span className="font-black text-sm text-blue-600">{currentBranch}</span>
                  <ChevronDown size={14} className="text-gray-400 group-hover:text-blue-500 transition-colors" />
                </div>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" className="w-56 p-2 bg-white rounded-xl shadow-2xl border border-slate-100 z-[100]">
                <div className="px-2 py-1.5 mb-1">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Select Branch</span>
                </div>
                {branches.map((branch: string) => (
                  <DropdownMenuItem
                    key={branch}
                    onClick={() => setCurrentBranch(branch)}
                    className={`flex items-center justify-between px-3 py-2.5 rounded-lg cursor-pointer transition-colors mb-0.5 ${currentBranch === branch ? 'bg-blue-50 text-blue-700' : 'hover:bg-slate-50 text-slate-700'
                      }`}
                  >
                    <div className="flex items-center gap-3">
                      <Building2 size={16} className={currentBranch === branch ? 'text-blue-600' : 'text-slate-400'} />
                      <span className="font-bold text-sm">{branch}</span>
                    </div>
                    {currentBranch === branch && <Check size={14} className="text-blue-600" />}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            </DropdownMenu>
